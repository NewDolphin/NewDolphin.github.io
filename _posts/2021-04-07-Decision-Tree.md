...
条件熵 $H(Y\mid X)$ 表示在已知随机变量X的条件下随机变量Y的不确定性。定义为：$H(Y\mid X)=\sum_{x\epsilon X}^{}p(x)\cdot H(Y\mid X=x)$。条件熵是指在给定某个数（某个变量为某个值）的情况下，另一个变量的不确定性(熵)是多少。[举例](https://breezepqf.cn/2020/07/12/decision-tree/)

样本集合D的熵定义为 $Ent(D)=-\sum_{k=1}^{\|y\|}p_{k} \cdot log_{2}(p_{k})$，D中包含 $\| y \|$ 个类别，$p_{k}$ 表示第k类样本所占比例，$Ent(D)$ 值越小，则D的纯度越高。
